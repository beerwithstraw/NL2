...
Scans NL2/ and NL2_Consolidated/ subfolders within each FY/Quarter folder.
"""
...
def scan(config: Dict[str, Any]) -> List[ScanResult]:
    """
    Walk the folder structure and return all NL-2 PDFs to be processed.
    """
    ...
    for fy in fiscal_years:
        ...
        for quarter in quarters:
            ...
            # --- Scan NL2/ subfolder ---
            nl2_path = os.path.join(q_path, "NL2")
            if os.path.isdir(nl2_path):
                for fname in os.listdir(nl2_path):
                    if not fname.lower().endswith(".pdf"):
                        continue
                    result = _extract_company_key(fname)
                    if result:
                        company_key, company_raw = result
                        pdf_path = os.path.join(nl2_path, fname)
            ...
            # --- Scan NL2_Consolidated/ subfolder ---
            consol_path = os.path.join(q_path, "NL2_Consolidated")
